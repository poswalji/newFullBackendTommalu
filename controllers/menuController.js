...
exports.getAllProducts = asyncHandler(async (req, res, next) => {
   ...
   if (storeId) {
      ...
      if (mongoose.Types.ObjectId.isValid(storeId)) {
         ...
         return next(new AppError('Invalid storeId', 400));
      }
   }
   ...
   const [menuItems, total] = await Promise.all([
      MenuItem.find(filter)
         .populate({
            ...
            select:
               'storeName category address phone isOpen rating status available openingTime closingTime',
         })
         .sort(sortOptions)
         .skip(skip)
...
exports.updateMenuItem = asyncHandler(async (req, res, next) => {
   ...
   if (req.body.isSpecial !== undefined) {
      req.body.isSpecial =
         req.body.isSpecial === 'true' || req.body.isSpecial === true;
   }
...
exports.toggleAvailability = asyncHandler(async (req, res, next) => {
   ...
   if (req.user && req.user.role === 'storeOwner') {
      ...
      if (!store || store.ownerId.toString() !== req.user._id.toString()) {
         return next(
            new AppError(
               'You do not have permission to toggle availability of this menu item',
               403
            )
         );
      }
...
exports.getMenuItemById = asyncHandler(async (req, res, next) => {
   ...
   const menuItem = await MenuItem.findById(menuItemId).populate(
      'storeId',
      'storeName category address phone isOpen rating'
   );
   ...
   if (req.user && req.user.role === 'storeOwner') {
      ...
      if (!store || store.ownerId.toString() !== req.user._id.toString()) {
         return next(
            new AppError(
               'You do not have permission to view this menu item',
               403
            )
         );
      }
